...
-- ============================================
-- STORED PROCEDURE untuk update overdue status
-- ============================================
DELIMITER $$
...
SELECT
    ...
    bk.isbn,
    CASE
        WHEN b.due_date < CURDATE() THEN DATEDIFF(CURDATE(), b.due_date)
        ELSE 0
    END as days_overdue
FROM borrowings b
...
SELECT
    ...
    SUM(CASE WHEN b.status = 'borrowed' THEN 1 ELSE 0 END) as active_borrowings,
    SUM(CASE WHEN b.status = 'returned' THEN 1 ELSE 0 END) as returned_books,
...
(1, 1, DATE_SUB(CURDATE(), INTERVAL 5 DAY), DATE_ADD(CURDATE(), INTERVAL 9 DAY), 'borrowed'),
...
-- ============================================
-- CREATE USER 'iterary_app'@'%' IDENTIFIED BY 'your_secure_password';
-- GRANT ALL PRIVILEGES ON iterary.* TO 'iterary_app'@'%';
-- FLUSH PRIVILEGES;
...
-- Stored Procedures: 1 (update_overdue_status)
